...
if (!isset($_SESSION['UserID'], $_SESSION['role']) || $_SESSION['role'] !== 'Admin') {
    ...
    header('Location: /HMS-main/auth/admin_login.php');
    exit();
}
...
if ($conn) {
    $stmt = $conn->prepare("SELECT full_name, email, ContactNumber, username FROM users WHERE UserID = ? AND role = 'Admin' LIMIT 1");
    if ($stmt) {
...
<style>
  /* Keep same styles as nurse_header */
  body {
    margin: 0;
...
<div class="header">
  ...
  <div class="right-section">
    ...
    <div class="user-dropdown" id="userDropdownToggle">
      <span>
        Admin: <?php echo htmlspecialchars($_SESSION['full_name'] ?? 'Unknown'); ?> <i class="fas fa-chevron-down"></i>
      </span>
  ...
  profileForm.addEventListener('submit', (e) => {
    ...
    .then(data => {
      if (data.success) {
        ...
        userSpan.childNodes[0].nodeValue = 'Admin: ' + data.full_name + ' ';
      } else {
